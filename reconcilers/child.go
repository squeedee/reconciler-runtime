...
func (r *ChildReconciler[T, CT, CLT]) SetupWithManager(ctx context.Context, mgr controllerruntime.Manager, bldr *builder.Builder) error {
	r.init()
...
func (r *ChildReconciler[T, CT, CLT]) reconcile(ctx context.Context, resource T) (CT, error) {
	...
	if len(items) == 1 {
		...
		for _, extra := range items {
			...
			if err := c.Delete(ctx, extra); err != nil {
				pc.Recorder.Eventf(resource, v1.EventTypeWarning, "DeleteFailed",
					"Failed to delete %s %q: %v", typeName(r.ChildType), extra.GetName(), err)
				return nilCT, err
			}
			pc.Recorder.Eventf(resource, v1.EventTypeNormal, "Deleted",
	...
	if err != nil {
		if errors2.Is(err, OnlyReconcileChildStatus) {
			return actual, nil
		}
		return nilCT, err
	}
	if !internal.IsNil(desired) {
		if !r.SkipOwnerReference {
			if err := controllerruntime.SetControllerReference(resource, desired, c.Scheme()); err != nil {
				return nilCT, err
			}
		}
...
func (r *ChildReconciler[T, CT, CLT]) ourChild(resource T, obj CT) bool {
	if !r.SkipOwnerReference && !v12.IsControlledBy(obj, resource) {
		return false
	}
